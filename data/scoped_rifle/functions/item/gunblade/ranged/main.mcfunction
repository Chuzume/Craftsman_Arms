...
# 発砲
    execute if entity @s[scores={S.Rif_Shot=0..}] unless data storage chuz:context Item.Mainhand.tag.ChuzData{Ammo:0} run function scoped_rifle:item/gunblade/ranged/fire/normal
    # 残弾0なら失敗
...
# チャージ
    ...
    # チャージショット
        execute if score @s[scores={S.Rif_Shot=0..}] S.Rif_Charge matches 40.. run function scoped_rifle:item/gunblade/ranged/fire/chargeshot

# 残弾0でエイムしてない場合はリロードをする
    execute unless data storage chuz:context Item.Mainhand.tag.ChuzData{Ammo:8} run scoreboard players add @s S.Rif_Ready 1
