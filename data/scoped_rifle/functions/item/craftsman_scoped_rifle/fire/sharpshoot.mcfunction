...
# 弾を召喚
    # 腰だめ
        ...
        execute if entity @s[tag=!Chuz.UsingSpyglass] if data storage chuz:context Item.Mainhand.tag.ChuzData{ItemID:Craftsman_Scoped_Rifle} anchored eyes run summon armor_stand ^-0.25 ^ ^ {Silent:1b,Marker:1b,Invisible:1b,Tags:["S.Rif_SharpBullet","S.Rif_Projectile","Chuz_UUIDAttack","S.Rif_Init"],ArmorItems:[{id:"minecraft:structure_void",Count:1b,tag:{ChuzData:{Damage:8.0,HeadDamage:18.0,Speed:90,Range:800}}},{},{},{}]}
        execute if entity @s[tag=!Chuz.UsingSpyglass] at @s run function scoped_rifle:item/craftsman_scoped_rifle/fire/spread

    # スコープ！
        execute if entity @s[tag=Chuz.UsingSpyglass] anchored eyes run particle smoke ^ ^ ^0.7 0 0 0 0.05 2
        execute if entity @s[tag=Chuz.UsingSpyglass] anchored eyes run summon armor_stand ^ ^ ^ {Silent:1b,Marker:1b,Invisible:1b,Tags:["S.Rif_SharpBullet","S.Rif_Projectile","Chuz_UUIDAttack","S.Rif_Init"],ArmorItems:[{id:"minecraft:structure_void",Count:1b,tag:{ChuzData:{Damage:8.0,HeadDamage:18.0,Speed:90,Range:800}}},{},{},{}]}
