...
# 弾チェックして持ってたスコア加算
    execute if score $Chuz.Setting S.Rif_Rule1 matches 1 store success score @s Chuz.Temporary run clear @s chain_command_block{ChuzData:{ItemID:Explosive_Cartridge}} 0
...
# ざくざくと弾を込める
    ...
    execute if score @s S.Rif_Reload matches 40.. in overworld run function scoped_rifle:item/grenade_launcher/reload/finish