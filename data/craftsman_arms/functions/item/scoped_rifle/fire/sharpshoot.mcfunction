...
# 弾を召喚
    ...
    # スコープ！
        ...
        execute if entity @s[tag=Chuz.UsingSpyglass] anchored eyes run summon armor_stand ^ ^ ^ {Silent:1b,Marker:1b,Invisible:1b,Tags:["S.Rif_SharpBullet","S.Rif_Projectile","Chuz_UUIDAttack","S.Rif_Init"],ArmorItems:[{id:"minecraft:structure_void",Count:1b,tag:{ChuzData:{Damage:8.0,HeadDamage:26.0,Speed:90,Range:800}}},{},{},{}]}
        execute if entity @s[tag=Chuz.UsingSpyglass] anchored eyes run summon marker ^ ^ ^10 {Tags:["S.Rif_Spread"]}

# 弾丸にInit処理
    tag @e[type=armor_stand,tag=S.Rif_Init,limit=1] add S.Rif_Bullet
