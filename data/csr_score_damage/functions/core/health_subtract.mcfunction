...
# Mobへ代入
    ...
    # ヘルスが1以上ならUUID攻撃
        execute unless entity @s[type=player] unless entity @s[type=ender_dragon] if score $SubtractedHealth ScoreDamageCore matches 1.. run function scoped_rifle:uuid_hit_no_kb
    ...
    # プレイヤーかつヘルス0なら死亡メッセージ
         execute if entity @s[type=player,tag=S.Rif_Hit.Bullet] if score @s ScoreToHealth matches ..0 run function mozambique:message/death/moza_bullet
        execute if entity @s[type=player,tag=S.Rif_Hit.BulletHS] if score @s ScoreToHealth matches ..0 run function mozambique:message/death/moza_bullet_headshot
# 演出
